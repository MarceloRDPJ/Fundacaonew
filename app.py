...
# Pega as chaves do ambiente do Render ou do seu arquivo .env local
...
def generate_elevenlabs_audio(text):
    if not ELEVENLABS_API_KEY or not VOICE_ID:
        print("ERRO: Chaves do ElevenLabs não foram encontradas no ambiente.")
        return None
    ...
    try:
        ...
        response.raise_for_status() # Lança um erro se a requisição falhar (como 401)
...
# (Suas funções normalize_text, find_best_intent, etc. continuam aqui)
def normalize_text(text):
    if not text: return ""
    return ''.join(c for c in unicodediata.normalize('NFD', text) if unicodedata.category(c) != 'Mn').lower()
...
def get_intent_by_tag(tag):
    for intent in knowledge_base['intents']:
        if intent['tag'] == tag: return intent
    return None
